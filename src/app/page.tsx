...
export default function Home() {
  return (
    <div className="flex flex-1 flex-col bg-cream">
      ...
      <section
        id="work"
        className="scroll-mt-24 border-b-2 border-ink px-4 py-16 sm:px-6 sm:py-20"
      >
          ...
          <div className="mt-12 space-y-14">
            {site.experiences.map((job) => (
              <article
                key={job.id}
                id={job.id}
                className="scroll-mt-24 border-2 border-ink bg-white shadow-hard"
                style={{ borderRadius: "var(--radius)" }}
              >
                <header className="border-b-2 border-ink px-6 py-6 sm:px-8 sm:py-7">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                      <h3 className="text-xl font-semibold uppercase text-ink sm:text-2xl">
                        {job.company}
                      </h3>
                  ...
                  <p className="mt-5 max-w-3xl text-sm leading-relaxed text-ink sm:text-base">
                    {job.summary}
                  </p>
                  {"note" in job && job.note ? (
                    <p className="mt-3 text-xs text-muted">{job.note}</p>
                  ) : null}
                  <ul className="mt-5 flex flex-wrap gap-2">
                    {job.stack.map((tech) => (
                      <li
                        key={tech}
                        className="border border-ink bg-cream px-2 py-1 text-[10px] font-medium uppercase text-ink"
                        style={{ borderRadius: "var(--radius)" }}
                      >
                        {tech}
                      </li>
                  ...
                  {job.themes.map((theme, index) => (
                    ...
                    >
                      <div>
                        ...
                        <h4 className="mt-2 text-base font-semibold uppercase leading-snug text-ink sm:text-lg">
                          {theme.title}
                        </h4>
                      ...
                      <ul className="space-y-3">
                        {theme.points.map((point) => (
                          <li
                            key={point}
                            className="border-l-2 border-sky pl-4 text-sm leading-relaxed text-ink"
                          >
                            {point}
                          </li>
                        ))}
        ...
        <div className="mx-auto max-w-300">
          ...
          <div className="mt-10 grid gap-8 md:grid-cols-3">
            {site.focuses.map((focus, index) => (
              <div key={focus.title} className="border-t-2 border-ink pt-4">
                <p className="text-[10px] font-semibold uppercase text-muted">
                  0{index + 1}
                </p>
                <h3 className="mt-2 text-base font-semibold uppercase text-ink">
                  {focus.title}
                </h3>
                <p className="mt-3 text-sm leading-relaxed text-ink">
                  {focus.body}
      ...
      <section
        ...
        className="scroll-mt-24 border-b-2 border-ink px-4 py-16 sm:px-6 sm:py-20"
      >
        <div className="mx-auto grid max-w-300 gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-stretch">
          <div>
            <p className="text-xs font-semibold uppercase text-muted">
              About
            </p>
            ...
            <dl className="mt-8 grid gap-3 sm:grid-cols-2">
              ...
              <div className="border border-ink/30 bg-white px-3 py-2">
                ...
                <dd className="mt-1 text-xs text-ink">{site.location}</dd>
              </div>
              <div className="border border-ink/30 bg-white px-3 py-2">
                <dt className="text-[10px] font-semibold uppercase text-muted">
                  LinkedIn
      ...
      >
        <div className="mx-auto flex max-w-300 flex-col items-start gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase text-muted">
              Contact
            </p>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <PopLift>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`inline-flex items-center justify-center border-2 border-ink bg-white px-6 py-2.5 text-sm font-medium text-ink ${LIFT_FACE}`}
                style={{ borderRadius: "var(--radius)" }}
              >
                LinkedIn
              </a>
      ...
      <footer className="border-t-2 border-ink bg-white px-4 py-6 sm:px-6">
        <div className="mx-auto flex max-w-300 flex-wrap items-center justify-between gap-3 text-xs text-muted">
          <p>
            © {new Date().getFullYear()} {site.name}
          </p>
          <Link
            href="/dev/component-explorer"
            className="uppercase underline underline-offset-2 hover:text-ink"
          >
            Component explorer
          </Link>
        </div>
